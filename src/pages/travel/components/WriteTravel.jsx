...
// WriteTravel 컴포넌트 정의
// 이 컴포넌트는 여행 목록을 새로 작성하거나 수정하는 폼을 제공한다.
const WriteTravel = ({ travelToEdit, onSave, onCancel }) => {
  // 입력 필드 상태를 관리
  const [title, setTitle] = useState('')
  const [content, setContent] = useState('')
  const [thumbnail, setThumbnail] = useState('')

  // travelToEdit 이 변경될 때마다 호출된다.
  // travelToEdit이 주어지면 해당 항목의 제목, 내용, 썸네일을 상태에 설정한다.
  useEffect(() => {
    if (travelToEdit) {
      setTitle(travelToEdit.title)
      setContent(travelToEdit.content)
      setThumbnail(travelToEdit.thumbnail)
    }
  }, [travelToEdit])
  ...
  return (
    <div className="bg-white p-4 shadow">
      {/* 제목 */}
      <h2 className="text-xl font-bold mb-4">{travelToEdit ? '수정하기' : '새 글 작성하기'}</h2>
