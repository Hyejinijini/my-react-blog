...
// WriteReview 컴포넌트 정의
const WriteReview = ({ reviewToEdit, onSave, onCancel }) => {
  // 제목 상태를 관리
  ...
  // reviewToEdit이 변경될 때마다 호출된다.
  // reviewToEdit이 존재하면 해당 항목의 제목, 내용, 썸네일을 상태에 설정한다.
  useEffect(() => {
    if (reviewToEdit) {
      setTitle(reviewToEdit.title)
      setContent(reviewToEdit.content)
      setThumbnail(reviewToEdit.thumbnail)
    }
  }, [reviewToEdit])
  ...
  return (
    <div className="bg-white p-4 shadow rounded mb-4">
      {/* 제목 */}
      <h2 className="text-xl font-bold mb-4">{reviewToEdit ? '수정하기' : '새 리뷰 작성하기'}</h2>
